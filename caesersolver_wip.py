...
for i in range(len(input_string)):
    ...
    if position_character in probability:
        prob = probability[position_character]
        print('probability of', position_character, prob)
    shift = (char_count-((prob*length)**2))/(prob*length)
    print("shift code", shift)
    
    
    #caeser cipher 
    newval = chr((ord(position_character)+shift)%26) + ord(99)
